...
void PCLFunc::SetUpGrid(int grid_size, float grid_step)
{
	...
	for (int j = 0; j <= grid_size; j++)
	{
		...
		if (j == grid_size / 2)
		{
			viewer->addLine(p1, p2, 0.25, 0.25, 0.25, name_line_x);//在指定视口 viewport 中添加一条线段,ptl 、 pt2 为线段起始和终点,深色
			//rgb 为颜色,id 为对象 ID 。 
		}
		else
		{
			viewer->addLine(p1, p2, 0.827451, 0.827451, 0.827451, name_line_x);//浅色
			px.x = p1.x;
...
void PCLFunc::AddCloudPointArray(float x[7200], float y[7200], float z[7200], float LaserPose[4], bool RSSIFlag, float RSSIData[7200], int ScanPointCount)
{
	...
	if (FlagPointDisplay == true)
	{
		...
		viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3, "cloud");//在指定的viewport内设置id模型对象的渲染属性
	}
	else
	{
